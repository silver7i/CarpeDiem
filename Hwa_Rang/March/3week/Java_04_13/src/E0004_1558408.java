...
public class E0004_1558408 {
    /////////////////////////////////
    // 메인 프로그램 통신 변수 정의
    /////////////////////////////////
    private static final String HOST = "70.12.60.68";
    ...
    public static void main(String[] args) {
        ...
        int[][] DIRS = {{-1,0}, {0,1}, {1,0}, {0,-1}};  // 상 우 하 좌
        String[] MOVE_CMDS = {"U A", "R A", "D A", "L A"};
        String[] FIRE_CMDS = {"U F", "R F", "D F", "L F"};
        String START_SYMBOL = "M";  // 탱크 (내 위치)
        String TARGET_SYMBOL = "X";  // 포탑 (파괴해야 하는 목표물)
        String WALL_SYMBOL = "R";  // 바위
    ...
    private static Queue<String> bfs(
            String[][] grid,  // 맵 정보
            int[] start,  // 탱크 위치 (내 위치)
            int[] target,  // 포탑 위치 (목표물)
            String wall,  // 바위
            int[][] dirs,  // 방향 전환
            String[] moveCmds,  // 이동 명령어
            String[] fireCmds) {  // 포탄 발사 명령어
        ...
        while (!queue.isEmpty()) {
            ...
            if(r == target[0] && Math.abs(c - target[1]) <= 3) {
            	for(int i = 1; i < target[1] - c; i++) {
            		if(grid[r][c+i].equals("R") || grid[r][c+i].equals("T")) break;
            		
            		if(i < target[1] - c - 1) continue;
            		Queue<String> result = new LinkedList<>(actions);
                    result.offer(fireCmds[1]);
                    return result;
            	}
            	
            } else if(c == target[1] && Math.abs(r - target[0]) <= 3) {
            	for(int i = 1; i < r - target[0]; i++) {
            		if(grid[r-i][c].equals("R") || grid[r-i][c].equals("T")) break;
            		
            		if(i < r - target[0] - 1) continue;
            		Queue<String> result = new LinkedList<>(actions);
                    result.offer(fireCmds[0]);
                    return result;
            	}
            }
            ...
            for (int d = 0; d < dirs.length; d++) {
                ...
                if(nr < 0 || nr >= rows || nc < 0 || nc >= cols) continue;  // 맵 범위를 벗어남
                if(grid[nr][nc] == null) continue;
                if(grid[nr][nc].equals(wall)) continue;  // 바위
                if(grid[nr][nc].equals("W")) continue;  // 물
                if(grid[nr][nc].equals("T")) continue;  // 나무
                if(visited.contains(key)) continue;  // 이미 방문함
