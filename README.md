...
### ❓ 파일 내용만 가져오기 vs 커밋 내역까지 가져오기

| 방법 | 명령어 | 특징 |
| :--- | :--- | :--- |
| **파일만 복사** | `git checkout [브랜치] -- [경로]` | 이력 없이 **현재 파일 상태**만 가져옴 (깔끔함) |
| **이력 포함** | `git cherry-pick [커밋해시]` | 해당 폴더의 **수정 내역(Commit)**까지 그대로 가져옴 |
---
|    **⚠️ 주의사항 (Cherry-pick 사용 시)** |    특정 폴더만 수정한 커밋이라도, 만약 그 커밋 안에 다른 파일의 수정이나 삭제(예: form, README.md 삭제 및 수정)가 섞여 있다면 **그 변화까지 통째로 가져오게 됩니다.** 따라서 **순수하게 특정 폴더의 결과물만** 안전하게 가져오고 싶을 때는 이력이 남지 않더라도 `checkout` 방식이 가장 충돌 적고 안전합니다.|
| :--- | :--- |
