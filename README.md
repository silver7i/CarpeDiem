...
**2. 내 브랜치 상태로 전체 복구 (방어)**            
- 머지로 인해 변해버린 내 브랜치(master)의 모든 상태를 일단 머지 전 상태(HEAD)로 되돌립니다. 이 단계에서 삭제된 파일들이 다시 살아납니다.         
- 명령어: 
    ```
    git checkout HEAD -- .
    ```
...
### ✨ 이 방식의 장점
- **이력 연결:**        
    `git log --graph` 확인 시 두 브랜치가 합쳐진 선이 남으며, 상대 브랜치의 커밋 내역이 내 브랜치 이력에 포함됩니다.
- **안전성:**       
    `checkout HEAD -- .` 과정을 통해 내가 원치 않는 파일 삭제나 수정을 원천 차단합니다.
- **잔디 보존:**        
    상대 브랜치에서 작업한 기여도가 내 브랜치 머지 과정에서도 유지됩니다.

---

#### 💡 주의사항
- **충돌(Conflict) 발생 시:** 위 과정 중 충돌이 나면 파일을 수정하고 git add . 를 한 뒤 다음 단계를 진행하세요.
- **커밋 전 확인:** git status를 입력하여 내가 가져오려던 폴더 외에 다른 파일이 변하지 않았는지 최종 확인 후 커밋하는 것이 좋습니다.
---
